...
(setq skw/d-base     user-emacs-directory
      skw/d-cache    (expand-file-name
                      "emacs/" (or (getenv "XDG_CACHE_HOME") "~/.cache"))
      ...
      skw/f-custom   (concat skw/d-cache "custom.el")
      skw/d-savedir  (concat skw/d-cache "saves/")
      skw/f-savehist (concat skw/d-cache "savehist"))
...
(setq package-user-dir           (concat skw/d-cache "elpa/")
      auto-save-list-file-prefix (concat skw/d-cache "auto-save-list/.saves-")
...
=cl-functions= byte-compiler warning is muted.
...
A few bindings below deliberately shadow Emacs defaults. They are collected
here so that nobody (future me included) mistakes them for accidents.

| Key       | Bound to                       | Default it replaces                  |
|-----------+--------------------------------+--------------------------------------|
| =C-v=     | =scroll-down=                  | =scroll-up-command=                  |
| =M-n=     | =scroll-up=                    | (unbound)                            |
| =C-z=     | =delete-other-windows=         | =suspend-frame=                      |
| =C-x C-l= | =goto-line=                    | =downcase-region= (see =C-x M-l=)    |
| =M-,=     | =er/expand-region=             | =xref-go-back=                       |
| =M-l=     | =downcase-word=                | =downcase-word= (unchanged)          |
| =M-o=     | =capitalize-word=              | =facemenu= prefix                    |
| =M-i=     | =upcase-word=                  | =tab-to-tab-stop=                    |
| =M-k=     | =skw/kill-to-bol=              | =kill-sentence=                      |
| =M-;=     | =skw/comment-or-uncomment-…=   | =comment-dwim=                       |
| =C-s=     | prefix (=C-s s=, =C-s r=, …)  | =isearch-forward= (use =C-s s=)      |
| =C-x u=   | =vundo=                        | =undo=                               |
| =C-x C-c= | kill terminal / server w/ =C-u= | =save-buffers-kill-terminal=        |
...
;; In case I happen to run a GUI Emacs #YouNeverKnow. This is done per frame
;; (and not with a one-shot `window-system' check at start-up) so that it also
;; applies to graphical frames created later by a daemon started from a tty.
...
(defun skw/setup-graphical-frame (&optional frame)
  "Apply GUI-only settings to FRAME."
  (with-selected-frame (or frame (selected-frame))
    (when (display-graphic-p)
      (global-hl-line-mode 1))))

(add-hook 'after-make-frame-functions #'skw/setup-graphical-frame)
(skw/setup-graphical-frame)
#+end_src
...
(use-package smart-mode-line
  ...
  (setq sml/shorten-directory t
        sml/shorten-modes t
        sml/no-confirm-load-theme t) ;; its theme is not in custom-safe-themes on a fresh machine
  :config
...
(defun skw/kill-completions-buffer ()
  "Get rid of the *Completions* buffer when leaving the minibuffer."
  (let ((buffer "*Completions*"))
    (and (get-buffer buffer) (kill-buffer buffer))))

(add-hook 'minibuffer-exit-hook #'skw/kill-completions-buffer)
#+end_src
...
(use-package ripgrep
  ...
  :custom-face
  (ripgrep-match-face ((t (:foreground "dodger blue" :background unspecified))))
  (ripgrep-hit-face ((t (:foreground "red")))))
#+end_src
...
=hunspell-{en_us,en_gb,fr,de}= dictionaries) on ArchLinux.
...
Emacs (≥ 28) has a built-in =undo-redo=; [[https://github.com/casouri/vundo][vundo]] visualizes the undo history as
